...
if __name__ == '__main__':
    ...
    # image_clouds = h5_file['image_clouds'][:].transpose(0, 2, 3, 1)
    # print(image_clouds.shape)
    ...
    for index in tqdm(indices):
        ...
        colors_pred = np.asarray(maps_color[2] * pcd_pred.shape[0]).reshape(-1, 3)
        dict_pcds = {name_png: [pcd_pred, colors_pred], str(name_png)+"_gt": [pcd_gt, colors_gt]}
        ...
        for key, value in dict_pcds.items():
            ...
            subprocess.call([f"{path_mitsuba}/mitsuba", path_xml, "-o", path_exr], stdout=subprocess.DEVNULL,
