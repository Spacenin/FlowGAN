...
class ShapeNetAllDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        sample.update({
            'image': self.images_file[self.part + '_images'][im_i] # (4, 137, 137)
        })
        ...
        if self.image_transform is not None:
            sample['image'] = self.image_transform(sample['image']) # (4, 224, 224)

        if self.cloud_transform is not None:
            sample = self.cloud_transform(sample) # (3, 2500)
